	...
	;September 6, 2024
	;Alex Wheelock
	;Program that uses a hex keypad with inputs from 0 to F on PORTB. To use the keypad, a ring counter must be generated using 4 bits from PORTB,
	;to test one row at a time. Depending on the high input on PORTB, corresponding with the row that has a high from the ring counter, is then
	;converted into an ASCII value on the output of PORTC going to the dot-matrix display.
...
;*** PORT B SETUP **** PORT B USED AS LOGIC INPUT FROM KEYPAD
		...
		BANKSEL	TRISB
		MOVLW	H'F0'				;SET PORTB PINS 0:3 AS OUTPUT, PINS 4:7 AS INPUT
		MOVWF	TRISB
		BANKSEL ANSELH
